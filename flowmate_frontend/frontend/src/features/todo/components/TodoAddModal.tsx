...
const TodoAddModal = ({
  projectId, username, isLeader, members, parts, onClose, onSuccess,
}: TodoAddModalProps) => {
  const [form, setForm] = useState<TodoCreateRequest>({
    ...
    todoPriority: "MID",
    todoDueAt: "",
    projectPartId: parts[0]?.id ?? 0,
    assigneeUsername: isLeader ? (members[0]?.username ?? "") : username,
  });
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      await createTodo(projectId, username, { ...form, todoDueAt: form.todoDueAt }, files);
      onSuccess();
      onClose();
    } catch (e: any) {
      console.error("투두 생성 실패", e);
      const msg = e?.response?.data?.message;
      showAlert(msg || "업무 추가에 실패했습니다.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h6 className="fw-semibold mb-0">업무 추가</h6>
        ...
        {/* 첨부파일 */}
        <div className="mb-4">
          <label className="form-label fw-medium small">첨부파일</label>
          <div className="d-flex align-items-center gap-2 p-1 rounded" style={{ border: "1px solid #ddd" }}>
            ...
            <span className="small text-muted">
              {files.length === 0 ? "선택된 파일이 없습니다." : `${files.length}개 선택됨`}
            </span>
          </div>
          {files.map((file, i) => (
